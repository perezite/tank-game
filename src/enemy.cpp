 ...
 * enemy.cpp
 *
 * Author   :   Fabian Gehri, Simon Schwegler
 * Date     :   09.05.2011
 *
 * All rights reserved.
 */
...
Enemy::Enemy() :
	Tank()
{
    // a little hack to generate pseudo-random numbers
    // GLUT seems to reset the rng generator, so we have to use this trick here
    // if we want to use our own seed
    static int rnd = 350;
    rnd = rnd+1;
    srand(rnd);
    ...
    std::cout << _initialPosition.x << std::endl;
